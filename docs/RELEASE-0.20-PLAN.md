...
- [ ] `handshake-at-origin`, `unplaceable-below-origin` and `mixed-derivation`
...
- [ ] The `retransmit` row says which act it names and what a copy of one
...
- [ ] `build.py` refuses a vector whose `.zpf` and `.jsonl` disagree, and was
      seen to refuse all three of Phase 1's defects on a scratch revert.
- [ ] Every single-file accept vector declares `extents`; the README says what
      the key asserts and that a SHOULD-report is not tested.
- [ ] `merge/` exists with its negative twin, a `RULES` entry names it, and
      `check.py` verifies the output against its inputs' holes.

The three:

- [ ] #80 and #106 are on a `0.21` milestone with a comment pointing at scope
      decision 1; #125 is on the milestone that decides D, pointing at scope
      decision 2.

Release:

- [ ] `python3 vectors/check.py` green; every vector stamps `0.20`;
      `reject-unknown-minor` rolled to `0/21` out of its own bytes. **55
      vectors, 35 options, 27 rules.**
- [ ] `NORMATIVE_ADDITIONS` exists, and every keyword the release added has an
...
- [ ] `CHANGELOG.md` `[0.20]` dated, with the three vectors named under `Fixed`.
...
*To be written at the tag, as `0.19`'s was. Things this plan expects to have got
wrong: the size of Phase 3, the number of `extents` that take real thought, and
whether the merge fixture's `gap` blocks look like signal or noise once written.*
