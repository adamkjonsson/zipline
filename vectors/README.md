...
| `advisory-below-origin-payload` | *(accept, **advisory**)* A below-origin record **carrying payload** — the shape whose cost the rule has to state out loud. `advisory-seq-start-below-origin`'s record is a zero-length `syn`, so placing it at zero width loses nothing and a reader that *dropped* it gives the same answers. With eight bytes the two readings diverge: under the rule the record is unplaceable at offset 0, its bytes excluded from the extent and from every coverage answer, while the record itself stays readable. One wrong reading places it at 4294967295, the other lists two records where the file has three. |
